...
public class Part5HiddenTest {
    ...
    public void test() {
        ...
        int[] input = new int[7 + random.nextInt(10)];

        for (int i = 0; i < input.length - 2; i++) {
            input[i] = random.nextInt(20);
        }
        input[input.length - 2] = -1;
        input[input.length - 1] = countEven(input);

        check(input, "Even");

        input[input.length - 1] = countOdd(input);
    ...
    private static int getLastNumber(String inputStr) {
        ...
        assertTrue("the output should contain a line of the type \"Even: 3\"", matcher.find());
    ...
    private String line(String out, String str) {
        ...
        fail("Your program should print the count of the even numbers.");
        return "";
    }

    private int countEven(int[] input) {
        int count = 0;
    ...
    private int countOdd(int[] input) {
        int count = 0;
